...
**Категории workflows**:
- **Управление клиентами** (9): Создание, обновление, управление данными клиентов
- **Извлечение данных** (11): Загрузка, импорт, обработка данных из различных источников
- **Создание контента** (10): Генерация объявлений, заголовков, текстов
- **Анализ** (20): Анализ ключевых слов, минус-слов, конкурентов, кампаний
- **Мониторинг** (1): Отслеживание статуса процессов
- **Утилиты** (2): Синхронизация с GitHub, webhook
- **Тестирование** (4): Тестовые workflows для отладки
- **Неопределенные** (7): Workflows без четкой категоризации
...
### 5.13 campaign-core-update.json (НОВЫЙ)

**Назначение**: Основной workflow для обновления кампаний с системой мониторинга и пропуска уже выполненных подпроцессов

**Основные этапы**:
1. **Инициализация**: Отметка начала процесса в `business_processes_states`
2. **Инициализация счетчика**: Установка `steps_passed = 0`
3. **Проверка выполнения подпроцессов**: SQL-запрос для проверки уже выполненных подпроцессов
4. **Условное выполнение**: Если подпроцесс не выполнен, вызывается соответствующий workflow
5. **Обновление прогресса**: Инкремент счетчика шагов
6. **Завершение**: Отметка завершения процесса

**Особенности**:
- **Система мониторинга**: Полная интеграция с `business_processes_states`
- **Пропуск выполненных процессов**: Механизм проверки `If SUB process already executed`
- **Динамические вызовы**: Вызов под-workflows по ID
- **Отслеживание прогресса**: Счетчик выполненных шагов

**SQL-запрос проверки выполнения**:
```sql
WITH cutoff AS (
...
SELECT s.business_process_state_id, s.business_process_id, s.started, s.finished, s.error_message, s.n8n_workflow_execution_id, s.n8n_workflow_root_execution_id
FROM main m
...
**Входные параметры**:
- `workflow_name` (string): Название под-workflow для проверки
- `id` (string): ID под-workflow для вызова

**Выходные данные**:
- Обновления в `business_processes_states`
- Результаты выполнения под-workflows

### 5.14 ADV_campaign_data_load.json (НОВЫЙ)

**Назначение**: Загрузка и обработка данных кампании из Dropbox

**Основные этапы**:
1. **Загрузка файла**: HTTP-запрос к Dropbox для получения CSV файла отчета
2. **Извлечение данных**: Парсинг CSV файла с разделителем ";"
3. **Обработка по батчам**: Разделение данных на части для обработки
4. **Вставка в БД**: Сохранение данных в соответствующие таблицы

**Особенности**:
- **Поддержка больших файлов**: Обработка по батчам через `splitInBatches`
- **Гибкая загрузка**: URL файла передается через переменную `yandex_report_dropbox_file_path`
- **CSV парсинг**: Автоматическое извлечение данных из CSV с разделителем ";"

**Входные параметры**:
- `yandex_report_dropbox_file_path` (string): URL файла отчета в Dropbox

**Выходные данные**:
- Данные кампании в соответствующих таблицах БД

### 5.15 ADV_campaign_get_service.json (НОВЫЙ)

**Назначение**: Получение и привязка услуг к записям кампании

**Основные этапы**:
1. **Обработка по батчам**: Цикл по записям кампании
2. **Обновление записей**: Привязка `service_id` к записям в `current_list`
3. **Связывание данных**: Связь записей кампании с услугами компании

**Особенности**:
- **Батчевая обработка**: Эффективная обработка больших объемов данных
- **Обновление связей**: Привязка услуг к записям кампании
- **Использование `current_list`**: Работа с временными данными анализа

**Входные данные**:
- Записи из `current_list` с полем `list_id`
- Данные об услугах с полем `service_id`

**Выходные данные**:
- Обновленные записи в `current_list` с привязанными `service_id`

### 5.16 clear-competitors-from-content.json (НОВЫЙ)

**Назначение**: Очистка контента от упоминаний конкурентов

**Основные этапы**:
1. **Установка переменных**: Установка `company_id` для обработки
2. **SQL-запрос очистки**: Удаление вариантов названий конкурентов с высокой схожестью
3. **Векторный поиск**: Использование векторных операций для поиска схожих фраз

**Особенности**:
- **Векторный анализ**: Использование PGVector для поиска схожих фраз
- **Порог схожести**: Удаление записей с схожестью <= 0.2
- **Связь с контентом**: Анализ относительно `company_content`
...
**Входные параметры**:
- `company_id` (string): ID компании для обработки

**Выходные данные**:
- Удаленные записи из `competitor_name_variants`
- Очищенный контент от упоминаний конкурентов

### 5.17 reset-test-processes-data.json (НОВЫЙ)

**Назначение**: Сброс тестовых данных процессов для отладки

**Основные этапы**:
1. **Установка переменных**: Установка `root_process_name` для сброса
2. **SQL-запрос сброса**: Принудительная установка ошибки для незавершенных процессов
3. **Очистка состояния**: Сброс состояния процессов для повторного тестирования

**Особенности**:
- **Тестовая утилита**: Предназначен для отладки и тестирования
- **Принудительный сброс**: Установка ошибки для всех незавершенных процессов
- **Каскадный сброс**: Сброс как основного процесса, так и всех подпроцессов

**SQL-запрос сброса**:
```sql
UPDATE business_processes_states 
SET error_happened = now(), error_message = 'test forced by user' 
WHERE finished IS NULL 
  AND error_happened IS NULL 
  AND business_process_id IN (
...
**Входные параметры**:
- `root_process_name` (string): Название корневого процесса для сброса

**Выходные данные**:
- Обновленные записи в `business_processes_states` с установленными ошибками
